...
struct MainModel {
    let network = SearchDKBlogNetwork()
    
    func searchDKBlog(_ query: String) -> Single<Result<DKBlog, SearchNetworkError>> {
        return network.searchDKBlog(query: query)
    }
    
    func getDKBlogValue(_ result: Result<DKBlog, SearchNetworkError>) -> DKBlog? {
    ...
    func getDKBlogError(_ result: Result<DKBlog, SearchNetworkError>) -> String? {
